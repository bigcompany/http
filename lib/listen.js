...
module['exports'] = function listen (options, callback) {
  ...
  var viewMiddle = function (req, res, next) {
    if (options.cacheView) {
      ...
      if (typeof app.whiteLabels[req.hostname] === "object") {
        console.log('using white label middleware'.yellow, req.hostname);
        return viewResource.middle({ view: app.whiteLabels[req.hostname] })(req, res, function(){
          // if we made it to next, it means we 404 on the custom white label view...
          // so we should revert back to the original base view
          viewResource.middle({ view: app.view })(req, res, next);
        });
      }
      // check if hostname does not match hook.io hostname
      // if the host name doesn't match, it means we have a white-labeled domain
      // we now need to use the custom view for this white-label if its available
  ...
  if (typeof options.view === "string") {
    viewResource.create({ path: options.view, autoindex: options.autoindex }, function (err, _view){
      ...
      view = _view;
      app.view = view;
      if (options.whiteLabels) {
        console.log('Using white labeled versions of the View'.blue);
        app.whiteLabels = {};
        if (options.whiteLabels instanceof Array) {
          var callbacks = options.whiteLabels.length;
          options.whiteLabelView = options.whiteLabelView || process.cwd() + "/white";
          options.whiteLabels.forEach(function (domain) {
            viewResource.create({ path: options.whiteLabelView + domain, autoindex: options.autoindex }, function (err, _view){
              if(err) throw err;
              callbacks--;
              console.log('loaded white label view'.yellow, domain)
              app.whiteLabels[domain] = _view;
              if (callbacks === 0) {
                finish();
              }
            });
          });
          // load each white label view
        } else {
          throw new Error('`whiteLabels` option must be an Array');
        }
      } else {
        finish();
      }
  ...
  function finish() {
    ...
    if (view) {
      //console.log('whiteLabels'.green, app.whiteLabels)
      app.use(viewMiddle);
    }
