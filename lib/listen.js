...
module['exports'] = function listen (options, callback) {
  ...
  var viewMiddle = function (req, res, next) {
    if (options.cacheView) {
      viewResource.middle({ view: app.view, autoindex: options.autoindex, secrets: options.secrets })(req, res, next);
    } else {
      viewResource.create({ path: options.view, autoindex: options.autoindex }, function (err, _view){
        if(err) throw err;
        view = _view;
        app.view = view;
        viewResource.middle({ view: app.view, secrets: options.secrets })(req, res, next);
      });
    }
  };
