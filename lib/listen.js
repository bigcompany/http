...
module['exports'] = function listen (options, callback) {
  ...
  if (typeof callback === 'undefined') {
    debug('warning: no callback provided to http.listen, using default callback.')
  ...
  if (options.auth && options.auth.basicAuth) {
    ...
    passport.use(new httpStrategy.BasicStrategy(
      function(userid, password, done) {
        if (options.authCallback) {
          options.authCallback({ userid: userid, password: password }, done);
        } else {
    ...
    app.use(passport.authenticate('basic', { session: true }), function (req, res, next){
      next();
    });
